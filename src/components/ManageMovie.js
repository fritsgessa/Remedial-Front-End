...
class ManageMovie extends Component {
    ...
    getMovieList = () => {
        axios.get('http://localhost:2019/movies-list')
        .then((res) => {
    ...
    onBtnAddClick = () => {
        ...
        if(nama === '' || tahun === '' ||  description === ''){
            ...
            axios.post('http://localhost:2019/movies-add', {
    ...
    onBtnDeleteClick = (id) => {
        if(window.confirm('Yakin nih bro?')){
            axios.post('http://localhost:2019/movies-delete/' + id)
                .then((res) => {
                    this.getMovieList();
                }).catch((err) => {
                    console.log(err)
    ...
    onBtnSaveClick = (id) => {
        ...
        axios.post('http://localhost:2019/movies-edit/' + id, {
            nama,tahun, description
        }).then((res) => {
